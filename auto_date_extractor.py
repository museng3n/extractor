...
class TelegramSignalExtractor:
    ...
    def parse_signal_message(self, message_text):
        ...
        if match:
            ...
            if 'NOW' in signal_text.upper():
                ...
                entries = [float(e) for e in re.findall(entry_pattern, signal_text, re.IGNORECASE)]
    ...
    def format_signal_for_posting(self, signal):
        ...
        """
        # Start with the symbol and direction
        formatted_text = f"{signal['symbol']} {signal['direction']}"
        
        # Add entry information based on entry type
        if signal['entry_type'] == 'market':
            formatted_text += " NOW"
        else:
            # Format limit entries
            if len(signal['entry_points']) > 0:
                formatted_text += f" limit from {signal['entry_points'][0]}"
                
                if len(signal['entry_points']) > 1 and 'direction2' in signal:
                    formatted_text += f" and {signal['direction2']} limit from {signal['entry_points'][1]}"
        
        # Add take profit targets
        take_profits = []
        for i in range(1, 5):  # Assume 4 take profit levels
            tp_key = f'tp{i}'
            if tp_key in signal['take_profits']:
                # Different formatting for first TP
                if i == 1:
                    take_profits.append(f"Tp{i} @ {signal['take_profits'][tp_key]} | {i}")
                else:
                    take_profits.append(f"Tp{i} @ {signal['take_profits'][tp_key]} {i}")
        
        # Add stop loss
        if 'stop_loss' in signal:
            if 'sl_level' in signal:
                stop_loss = f"SI @ {signal['stop_loss']} {signal['sl_level']}"
            else:
                stop_loss = f"SI @ {signal['stop_loss']}"
        else:
            stop_loss = ""
